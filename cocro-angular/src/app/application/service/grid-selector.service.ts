...
import {
  ...
  resizeGrid,
  withUpdatedCell,
} from '@domain/services/grid-utils.service';
...
export class GridSelectorService {
  ...
  selectOnClick(x: number, y: number): void {
    ...
    } else if (this.direction() === 'NONE' && isCellLetter(cell)) {
      // No adjacent clue hint and no direction set yet: default to rightwards
      this.direction.set('RIGHTWARDS');
    }
