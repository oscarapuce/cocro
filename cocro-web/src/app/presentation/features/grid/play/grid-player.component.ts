...
export class GridPlayerComponent implements OnInit, OnDestroy {
    ...
    this.joinSession.execute(shareCode).subscribe({
      next: (fullDto: SessionFullResponse) => {
        ...
        this.activePlayers.set([
          this.myUserId(),
          ...Array.from({ length: ghostCount }, (_, i) => `ghost-${i}`),
        ]);
  ...
  ngOnDestroy(): void {
    if (this.shareCode() && this.connected() && !this.leaving) {
      this.leaving = true;
    ...
    switch (event.type) {
      ...
      case 'ParticipantJoined': {
        ...
        this.activePlayers.update(ids => {
          ...
          if (ghostIdx !== -1) {
            const next = [...ids];
            next[ghostIdx] = joined.userId;
            return next;
          }
          return [...ids, joined.userId];
        });
        break;
      }
      case 'ParticipantLeft': {
        ...
        this.activePlayers.update(ids => {
          ...
          const withoutGhost = [...ids];
          const ghostIdx = withoutGhost.findIndex(id => id.startsWith('ghost-'));
          if (ghostIdx !== -1) withoutGhost.splice(ghostIdx, 1);
          return withoutGhost;
        });
        break;
      }
